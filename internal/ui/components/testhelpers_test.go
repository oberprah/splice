...
// The goldenFileName should be relative to the testdata directory
func assertGolden(t *testing.T, output, goldenFileName string, updateFlag bool) {
	t.Helper()

	// Navigate up to states package testdata since components don't have their own
	goldenPath := filepath.Join("..", "states", "testdata", goldenFileName)
