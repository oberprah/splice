...
pub fn parse_file_changes(numstat: &str, name_status: &str) -> Result<Vec<FileChange>, String> {
    ...
    for line in numstat.lines() {
        ...
        let additions = if is_binary { 0 } else { additions_str.parse().unwrap_or(0) };
        let deletions = if is_binary { 0 } else { deletions_str.parse().unwrap_or(0) };
    ...
    for line in name_status.lines() {
        ...
        let (additions, deletions, is_binary) = stats.get(&path)
